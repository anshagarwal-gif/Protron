...
public interface BudgetLineRepository extends JpaRepository<BudgetLine, Integer> {
    ...
    @Query(value = "SELECT * FROM budget_line WHERE tenant_id = :tenantId and end_timestamp is NULL", nativeQuery = true)
    List<BudgetLine> findByTenantIdAndEndTimestampIsNull(@Param("tenantId") String tenantId);

    /**
     * Find budget lines by budget owner
     */
    @Query(value = "SELECT * FROM budget_line WHERE budget_owner = :budgetOwner and end_timestamp is NULL", nativeQuery = true)
    List<BudgetLine> findByBudgetOwner(@Param("budgetOwner") String budgetOwner);

    /**
     * Find budget lines by sponsor
     */
    @Query(value = "SELECT * FROM budget_line WHERE sponsor = :sponsor and end_timestamp is NULL", nativeQuery = true)
    List<BudgetLine> findBySponsor(@Param("sponsor") String sponsor);

    /**
     * Find budget lines by currency
     */
    @Query(value = "SELECT * FROM budget_line WHERE currency = :currency and end_timestamp is NULL", nativeQuery = true)
    List<BudgetLine> findByCurrency(@Param("currency") String currency);

    /**
     * Find budget lines by LOB (Line of Business)
     */
    @Query(value = "SELECT * FROM budget_line WHERE lob = :lob and end_timestamp is NULL", nativeQuery = true)
    List<BudgetLine> findByLob(@Param("lob") String lob);

    /**
     * Find budget lines by budget name containing specific text (case insensitive)
     */
    @Query(value = "SELECT * FROM budget_line WHERE LOWER(budget_name) LIKE LOWER(CONCAT('%', :budgetName, '%')) and end_timestamp is NULL", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT * FROM budget_line WHERE tenant_id = :tenantId AND budget_owner = :budgetOwner and end_timestamp is NULL", nativeQuery = true)
    List<BudgetLine> findByTenantIdAndBudgetOwnerAndEndTimestampIsNull(@Param("tenantId") String tenantId,
            @Param("budgetOwner") String budgetOwner);

    /**
     * Find budget lines where amount approved is greater than specified amount
     */
    @Query(value = "SELECT * FROM budget_line WHERE amount_approved > :amount and end_timestamp is NULL", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT * FROM budget_line WHERE amount_available > :amount and end_timestamp is NULL", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT * FROM budget_line WHERE tenant_id = :tenantId AND currency = :currency and end_timestamp is NULL", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT * FROM budget_line WHERE budget_end_date > CURRENT_DATE and end_timestamp is NULL", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT * FROM budget_line WHERE attachment IS NOT NULL AND LENGTH(attachment) > 0 and end_timestamp is NULL", nativeQuery = true)
    List<BudgetLine> findBudgetLinesWithAttachments();

    /**
     * Get total approved amount by tenant ID
     */
    @Query(value = "SELECT COALESCE(SUM(amount_approved), 0) FROM budget_line WHERE tenant_id = :tenantId and end_timestamp is NULL", nativeQuery = true)
    BigDecimal getTotalApprovedAmountByTenant(@Param("tenantId") String tenantId);

    /**
     * Get total utilized amount by tenant ID
     */
    @Query(value = "SELECT COALESCE(SUM(amount_utilized), 0) FROM budget_line WHERE tenant_id = :tenantId and end_timestamp is NULL", nativeQuery = true)
    BigDecimal getTotalUtilizedAmountByTenant(@Param("tenantId") String tenantId);

    /**
     * Get total available amount by tenant ID
     */
    @Query(value = "SELECT COALESCE(SUM(amount_available), 0) FROM budget_line WHERE tenant_id = :tenantId and end_timestamp is NULL", nativeQuery = true)
    BigDecimal getTotalAvailableAmountByTenant(@Param("tenantId") String tenantId);

    /**
     * Check if budget line exists by budget line item
     */
    @Query(value = "SELECT COUNT(*) > 0 FROM budget_line WHERE budget_line_item = :budgetLineItem and end_timestamp is NULL", nativeQuery = true)
    boolean existsByBudgetLineItem(@Param("budgetLineItem") String budgetLineItem);

    /**
     * Find budget line by budget line item
     */
    @Query(value = "SELECT * FROM budget_line WHERE budget_line_item = :budgetLineItem LIMIT 1 and end_timestamp is NULL", nativeQuery = true)
