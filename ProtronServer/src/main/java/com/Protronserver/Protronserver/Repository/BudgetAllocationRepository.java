...
public interface BudgetAllocationRepository extends JpaRepository<BudgetAllocation, Integer> {
     ...
     */
    @Query(value = "SELECT * FROM budget_allocation WHERE tenant_id = :tenantId", nativeQuery = true)
    List<BudgetAllocation> findByTenantId(@Param("tenantId") String tenantId);

    /**
     * Find budget allocations by vendor name
     */
    @Query(value = "SELECT * FROM budget_allocation WHERE vendor_name = :vendorName", nativeQuery = true)
    List<BudgetAllocation> findByVendorName(@Param("vendorName") String vendorName);

    /**
     * Find budget allocations by system name
     */
    @Query(value = "SELECT * FROM budget_allocation WHERE system_name = :systemName", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT * FROM budget_allocation WHERE vendor_name = :vendorName AND system_name = :systemName", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT * FROM budget_allocation WHERE tenant_id = :tenantId AND vendor_name = :vendorName", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT * FROM budget_allocation WHERE amount > :amount", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT * FROM budget_allocation WHERE amount BETWEEN :minAmount AND :maxAmount", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT COALESCE(SUM(amount), 0) FROM budget_allocation WHERE budget_id = :budgetLineId", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT COALESCE(SUM(amount), 0) FROM budget_allocation WHERE tenant_id = :tenantId", nativeQuery = true)
    BigDecimal getTotalAllocationAmountByTenant(@Param("tenantId") String tenantId);

    /**
     * Get total allocation amount by vendor name
     */
    @Query(value = "SELECT COALESCE(SUM(amount), 0) FROM budget_allocation WHERE vendor_name = :vendorName", nativeQuery = true)
    BigDecimal getTotalAllocationAmountByVendor(@Param("vendorName") String vendorName);

    /**
     * Get total allocation amount by system name
     */
    @Query(value = "SELECT COALESCE(SUM(amount), 0) FROM budget_allocation WHERE system_name = :systemName", nativeQuery = true)
    BigDecimal getTotalAllocationAmountBySystem(@Param("systemName") String systemName);

    /**
     * Count allocations by budget line ID
     */
    @Query(value = "SELECT COUNT(*) FROM budget_allocation WHERE budget_id = :budgetLineId", nativeQuery = true)
    Long countAllocationsByBudgetLineId(@Param("budgetLineId") Integer budgetLineId);

    /**
     * Count allocations by vendor name
     */
    @Query(value = "SELECT COUNT(*) FROM budget_allocation WHERE vendor_name = :vendorName", nativeQuery = true)
    Long countByVendorName(@Param("vendorName") String vendorName);

    /**
     * Count allocations by tenant ID
     */
    @Query(value = "SELECT COUNT(*) FROM budget_allocation WHERE tenant_id = :tenantId", nativeQuery = true)
    Long countByTenantId(@Param("tenantId") String tenantId);

    /**
     * Find budget allocations with remarks containing specific text
     */
    @Query(value = "SELECT * FROM budget_allocation WHERE LOWER(remarks) LIKE LOWER(CONCAT('%', :remarks, '%'))", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT COUNT(*) > 0 FROM budget_allocation WHERE vendor_name = :vendorName AND system_name = :systemName", nativeQuery = true)
     ...
     */
    @Query(value = "SELECT vendor_name, COUNT(*) as allocation_count, SUM(amount) as total_amount, AVG(amount) as avg_amount, MIN(amount) as min_amount, MAX(amount) as max_amount FROM budget_allocation WHERE tenant_id = :tenantId GROUP BY vendor_name ORDER BY total_amount DESC", nativeQuery = true)
    List<Object[]> getAllocationSummaryByVendor(@Param("tenantId") String tenantId);

    /**
     * Get allocation summary by system
     */
    @Query(value = "SELECT system_name, COUNT(*) as allocation_count, SUM(amount) as total_amount, AVG(amount) as avg_amount FROM budget_allocation WHERE tenant_id = :tenantId GROUP BY system_name ORDER BY total_amount DESC", nativeQuery = true)
    List<Object[]> getAllocationSummaryBySystem(@Param("tenantId") String tenantId);

    /**
     * Find top N vendors by allocation amount
     */
    @Query(value = "SELECT vendor_name, SUM(amount) as total_amount FROM budget_allocation WHERE tenant_id = :tenantId GROUP BY vendor_name ORDER BY total_amount DESC LIMIT ?1", nativeQuery = true)
    List<Object[]> findTopVendorsByAmount(@Param("tenantId") String tenantId, int limit);

    /**
     * Get monthly allocation trend
     */
    @Query(value = "SELECT DATE_FORMAT(bl.start_timestamp, '%Y-%m') as month, COUNT(ba.allocation_id) as allocation_count, SUM(ba.amount) as total_amount FROM budget_allocation ba JOIN budget_line bl ON ba.budget_id = bl.budget_id WHERE ba.tenant_id = :tenantId AND bl.start_timestamp >= DATE_SUB(CURRENT_DATE, INTERVAL 12 MONTH) GROUP BY DATE_FORMAT(bl.start_timestamp, '%Y-%m') ORDER BY month", nativeQuery = true)
