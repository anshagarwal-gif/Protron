...
public interface BudgetAllocationRepository extends JpaRepository<BudgetAllocation, Integer> {
         ...
         */
        @Query(value = "SELECT vendor_name, SUM(amount) as total_amount FROM budget_allocation WHERE tenant_id = :tenantId GROUP BY vendor_name ORDER BY total_amount DESC", nativeQuery = true)
        List<Object[]> findTopVendorsByAmount(@Param("tenantId") String tenantId);

        /**
         * Get monthly allocation trend (simplified version)
         */
        @Query(value = "SELECT 'Current Month' as month, COUNT(allocation_id) as allocation_count, SUM(amount) as total_amount FROM budget_allocation WHERE tenant_id = :tenantId", nativeQuery = true)
